...
void md5_tttt(array<uint32_t, 4>& state, array<byte, 64>& block) {
    uint32_t a = state[0];
    uint32_t b = state[1];
    uint32_t c = state[2];
    uint32_t d = state[3];
...
void md5(const string& str) {
    ...
    for (auto& c : op) {
        format_to(back_inserter(output), "{:02x}", c);
    }
    println("{}", output);
}
...
int main() {
    md5(""s);
    md5("The quick brown fox jumps over the lazy dog"s);
    return 0;
}
